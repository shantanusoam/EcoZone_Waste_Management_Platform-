...
import type { PostGISPoint, Tables } from "@ecozone/types";
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Bin[]> => {
      ...
      return ((data || []) as Tables<"bins">[]).map((bin) => {
        const location = bin.location as PostGISPoint | null;
        return {
          ...bin,
...
export function useBin(id: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Bin | null> => {
      ...
      const location = binData.location as PostGISPoint | null;
      return {
        ...binData,
        lat: location?.coordinates[1] ?? 0,
        lng: location?.coordinates[0] ?? 0,
